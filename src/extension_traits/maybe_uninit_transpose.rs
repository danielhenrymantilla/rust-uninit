...
pub trait MaybeUninitTranspose {
    ...
    where &'a Self: MaybeUninitTranspose<Transposed = &'a Self::Transposed> {
        self.transpose()
    }
    ...
    where &'a mut Self: MaybeUninitTranspose<Transposed = &'a mut Self::Transposed> {
        self.transpose()
    }
}
...
impl<'a, T: 'a, const N: usize> MaybeUninitTranspose for &'a [MaybeUninit<T>; N] {
    ...
    fn transpose(self) -> Self::Transposed {
        ...
        //   `size_of::<T>() * N` and the align of `T`.
        unsafe {
            &*(self as *const [MaybeUninit<T>; N] as *const MaybeUninit<[T; N]>)
        }
    }
}
...
impl<'a, T: 'a, const N: usize> MaybeUninitTranspose for &'a mut [MaybeUninit<T>; N] {
    ...
    fn transpose(self) -> Self::Transposed {
        ...
        //   `size_of::<T>() * N` and the align of `T`.
        unsafe {
            &mut *(self as *mut [MaybeUninit<T>; N] as *mut MaybeUninit<[T; N]>)
        }
    }
}
