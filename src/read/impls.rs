...
default unsafe impl<R: Read> ReadIntoUninit for R {
    #[inline]
    default fn read_into_uninit<'buf>(
        self: &'_ mut Self,
        buf: Out<'buf, [u8]>,
    ) -> io::Result<&'buf mut [u8]> {
        let buf = buf.fill_with_iter(::core::iter::repeat(0));
        self.read(buf).map(move |n| &mut buf[..n])
    }

    #[inline]
    default fn read_into_uninit_exact<'buf>(
        self: &'_ mut Self,
        buf: Out<'buf, [u8]>,
    ) -> io::Result<&'buf mut [u8]> {
        let buf = buf.fill_with_iter(::core::iter::repeat(0));
...
unsafe impl<R: ReadIntoUninit + ?Sized> ReadIntoUninit for &'_ mut R {
    #[inline]
...
//
//   - `read_into_uninit_exact` is not overriden
unsafe impl ReadIntoUninit for &'_ [u8] {
    #[inline]
    fn read_into_uninit<'buf>(
        self: &'_ mut Self,
        buf: Out<'buf, [u8]>,
...
//
//   - `read_into_uninit_exact` is not overriden
unsafe impl<R: ReadIntoUninit + ?Sized> ReadIntoUninit for Box<R> {
    #[inline]
    fn read_into_uninit<'buf>(
        self: &'_ mut Self,
        buf: Out<'buf, [u8]>,
